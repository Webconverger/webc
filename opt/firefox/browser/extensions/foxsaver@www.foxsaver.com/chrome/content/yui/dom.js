(function() {
    var Y = FoxSaver.YUI.util, getStyle, setStyle, id_counter = 0, propertyCache = {}, reClassNameCache = {}, document = window.document;
    var isOpera = FoxSaver.YUI.env.ua.opera, isSafari = FoxSaver.YUI.env.ua.webkit, isGecko = FoxSaver.YUI.env.ua.gecko, isIE = FoxSaver.YUI.env.ua.ie;
    ...
    if (isIE) {
        setStyle = function(el, property, val) {
            switch (property) {
                case"opacity":
                    if (FoxSaver.YUI.lang.isString(el.style.filter)) {
    ...
    FoxSaver.YUI.util.Dom = {get: function(el) {
            if (el && (el.tagName || el.item)) {
                return el
            }
            if (FoxSaver.YUI.lang.isString(el) || !el) {
            ...
            var f = function(el) {
                ...
                el.className = FoxSaver.YUI.lang.trim([el.className, className].join(" "));
                return true
            };
            return Y.Dom.batch(el, f, Y.Dom, true)
            ...
            var f = function(el) {
                ...
                el.className = FoxSaver.YUI.lang.trim(el.className);
                return true
            };
            return Y.Dom.batch(el, f, Y.Dom, true)
...
FoxSaver.YUI.util.Region = function(t, r, b, l) {
    this.top = t;
    this[1] = t;
    this.right = r;
    this.bottom = b;
...
FoxSaver.YUI.util.Region.prototype.contains = function(region) {
    return(region.left >= this.left && region.right <= this.right && region.top >= this.top && region.bottom <= this.bottom)
};
FoxSaver.YUI.util.Region.prototype.getArea = function() {
    return((this.bottom - this.top) * (this.right - this.left))
};
FoxSaver.YUI.util.Region.prototype.intersect = function(region) {
...
FoxSaver.YUI.util.Region.prototype.union = function(region) {
...
FoxSaver.YUI.util.Region.prototype.toString = function() {
    return("Region {top: " + this.top + ", right: " + this.right + ", bottom: " + this.bottom + ", left: " + this.left + "}")
};
FoxSaver.YUI.util.Region.getRegion = function(el) {
    var p = FoxSaver.YUI.util.Dom.getXY(el);
    var t = p[1];
...
FoxSaver.YUI.util.Point = function(x, y) {
    if (FoxSaver.YUI.lang.isArray(x)) {
        y = x[1];
        x = x[0]
    }
...
FoxSaver.YUI.util.Point.prototype = new FoxSaver.YUI.util.Region();
FoxSaver.YUI.register("dom", FoxSaver.YUI.util.Dom, {version: "2.4.0", build: "733"})