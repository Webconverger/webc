...
unless(defined(&_BITS_PTHREADTYPES_H)) {
	...
	eval 'sub __PTHREAD_MUTEX_HAVE_ELISION () {1;}' unless defined(&__PTHREAD_MUTEX_HAVE_ELISION);
    } else {
	eval 'sub __spins () { ($d->{__espins});}' unless defined(&__spins);
	eval 'sub __elision () { ($d->{__elision});}' unless defined(&__elision);
	eval 'sub __PTHREAD_MUTEX_HAVE_ELISION () {2;}' unless defined(&__PTHREAD_MUTEX_HAVE_ELISION);
    }
